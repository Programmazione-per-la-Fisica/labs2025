...
Inoltre vogliamo raggruppare tali risultati in un'unica struttura dati: `Result`.
...
>
> ```bash
> g++-15 regression.cpp -Wall -Wextra -o regression
> ```
...
Procediamo quindi definendo una `struct`, che chiamiamo `Result`, in modo che includa quei campi.
...
Pensiamo quindi a come implementare correttamente i due metodi, così che **facciano quello che vogliamo**.

Nel caso dell'implementazione di una classe come questa, la cosa implica chiedersi: _cosa abbiamo bisogno di 
memorizzare, in termini di variabili membro (private), per raggiungere i nostri scopi?_
...
Facciamo un'ultima modifica all'interfaccia della classe, dichiarando `fit` come `const` in quanto **non modifica** le
variabili membro.
...
Come menzionato in aula, la soluzione dell'esercizio proposto sarà discussa in classe all'inizio del secondo ciclo di
lezioni.
...
[questo link per A-L](https://virtuale.unibo.it/mod/assign/view.php?id=2221995) e [questo link per M-Z](https://virtuale.unibo.it/mod/assign/view.php?id=2092150), il quale prevede il solo caricamento di file
